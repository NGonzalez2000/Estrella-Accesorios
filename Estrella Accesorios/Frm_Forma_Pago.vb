Public Class Frm_Forma_Pago
    ...
    Private Sub Limpiar()
        D_Descripcion.Text = ""
		D_Recargo.Text = Format(0, "##,##0.00")
		D_Descuento.Text = Format(0, "##,##0.00")
		D_Habilitado.Checked = False
	End Sub

    Private Sub Frm_Forma_Pago_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Listado.Items.Clear()
		SQL = "SELECT * FROM Forma_Pago ORDER BY fpa_descripcion ASC"
		If Base.Ejecutar_SQL(SQL, Tabla) Then
			Do While Tabla.Read
				'add items to ListView
				Itm = New ListViewItem With {
					.Text = Tabla.Item("fpa_id").ToString'Codigo
					}
				Itm.SubItems.Add(Trim(Tabla.Item("fpa_descripcion"))) 'Descripción
				Itm.SubItems.Add(Format(Tabla.Item("fpa_recargo"), "##,##0.00")) 'Recargo
				Itm.SubItems.Add(Format(Tabla.Item("fpa_descuento"), "##,##0.00")) 'Descuento
				Itm.SubItems.Add(IIf(Tabla.Item("fpa_habilitado"), "SI", "NO")) 'Habilitado
				Itm.SubItems(4).Tag = Tabla.Item("fpa_habilitado")
	...
	Private Sub Agregar_Click(sender As Object, e As EventArgs) Handles Agregar.Click
		...
		Else
			...
			If MsgBox("¿Esta seguro que desea grabar los datos?", vbQuestion + vbYesNo + vbDefaultButton2, "Confirmación") = MsgBoxResult.Yes Then
				...
				Dim C_C As Int16 = Proximo_Codigo("fpa_id", "Forma_Pago")

				If C_C <> 0 Then
					Dim Sql As String = "INSERT INTO Forma_Pago
											(fpa_id, fpa_descripcion, fpa_recargo, fpa_descuento, fpa_habilitado)
										VALUES
											( " + C_C.ToString + "
											,'" + Trim(D_Descripcion.Text) + "' 
											," + Replace(CCdec(D_Recargo.Text).ToString, ",", ".") + "
											," + Replace(CCdec(D_Descuento.Text).ToString, ",", ".") + "
											," + D_Habilitado.Checked.ToString + ")"

					If Base.Ejecutar_SQL(Sql) Then
						MsgBox("Forma de Pago agregada exitosamente!", vbInformation)
						...
						Itm.SubItems.Add(D_Recargo.Text) 'Recargo
						Itm.SubItems.Add(D_Descuento.Text) 'Descuento
						Itm.SubItems.Add(IIf(D_Habilitado.Checked, "SI", "NO")) 'Habilitado
						Itm.SubItems(4).Tag = D_Habilitado.Checked
					...
					End If
				Else
					MsgBox("Se produjo un error al generar el código de Forma de Pago")
				End If
			End If
		End If
	End Sub
	...
	Private Sub Modificar_Click(sender As Object, e As EventArgs) Handles Modificar.Click
		If Modificar.Text = "Modificar" Then
			...
			D_Recargo.Text = Listado.SelectedItems(0).SubItems(Col_Recargo.Index).Text
			D_Descuento.Text = Listado.SelectedItems(0).SubItems(Col_Descuento.Index).Text
		...
		Else
			...
			If MsgBox("¿Esta seguro que desea grabar los datos?", vbQuestion + vbYesNo + vbDefaultButton2, "Confirmación") = MsgBoxResult.Yes Then
				...
				Dim Sql As String = "UPDATE Forma_Pago
										SET  fpa_descripcion = '" + Trim(D_Descripcion.Text) + "' 
											,fpa_recargo = " + Replace(CCdec(D_Recargo.Text).ToString, ",", ".") + "
											,fpa_descuento = " + Replace(CCdec(D_Descuento.Text).ToString, ",", ".") + "
											,fpa_habilitado = " + D_Habilitado.Checked.ToString + "
										WHERE fpa_id = " + D_Descripcion.Tag

				If Base.Ejecutar_SQL(Sql) Then
					MsgBox("Foma de pago modificada exitosamente!", vbInformation)

					With Listado.SelectedItems(0)
						.SubItems(Col_Descripcion.Index).Text = Trim(D_Descripcion.Text)
						.SubItems(Col_Recargo.Index).Text = D_Recargo.Text
						.SubItems(Col_Descuento.Index).Text = D_Descuento.Text
						.SubItems(Col_Habilitado.Index).Text = IIf(D_Habilitado.Checked, "SI", "NO") 'Habilitado
	...
	Private Sub Borrar_Click(sender As Object, e As EventArgs) Handles Borrar.Click
		...
		If MsgBox("¿Esta seguro que desea brorrar el registro seleccionado?" & vbCrLf & Listado.SelectedItems(0).SubItems(1).Text, vbQuestion + vbYesNo + vbDefaultButton2) = vbYes Then
			Sql = "DELETE * FROM Forma_Pago WHERE fpa_id = " + Listado.SelectedItems(0).SubItems(0).Text
	...
	Private Sub D_Descuento_Validated(sender As Object, e As EventArgs) Handles D_Descuento.Validated
		If D_Descuento.Text = "" Then D_Descuento.Text = "0"
		D_Descuento.Text = Format(CCdec(D_Descuento.Text), "##,##0.00")
	End Sub

	Private Sub D_Recargo_Validated(sender As Object, e As EventArgs) Handles D_Recargo.Validated
		If D_Recargo.Text = "" Then D_Recargo.Text = "0"
		D_Recargo.Text = Format(CCdec(D_Recargo.Text), "##,##0.00")
	End Sub
